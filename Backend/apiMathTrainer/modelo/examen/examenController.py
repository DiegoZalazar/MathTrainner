...
def Post(event):
    ...
    try:
        ...
        rows = examen_dao.UpdateExamenInfo(data={
            "idEstudiante": estudianteAvance["idEstudiante"],
            "realizado": 1
        })
        status_code = 200
...
def GetExamenInfo(event):
    global status_code, response_body, examen_dao
    data = {}
    try:
        data["idEstudiante"] = event['requestContext']['authorizer']['claims']['sub']
        # Create new instance DAO
        examen_dao = ExamenDAO(os.environ['HOST'], os.environ['USER'], os.environ['PASSWORD'], os.environ['DB'])
        response_body = examen_dao.GetExamenInfo(data)
        status_code = 200
    except Exception as e:
        response_body = {'message': f'Internal Server Error: {e}'}
        status_code = 500
